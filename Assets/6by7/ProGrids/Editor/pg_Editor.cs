...
#region PREFERENCES
	...
	public void LoadPreferences()
	{
		...
		if(EditorPrefs.HasKey(pg_Constant.GridUnit))
			SetSnapValue( (SnapUnit)EditorPrefs.GetInt(pg_Constant.GridUnit), EditorPrefs.GetFloat(pg_Constant.SnapValueEditorPref));
...
#region MENU
	...
	[MenuItem("Tools/ProGrids/Cycle SceneView Projection _\\", false, 101)]
	public static void CyclePerspective()
	{
		if(instance == null) return;	//  todo - remove when verify shortcut stops acting like a buggy turd.
	...
	[MenuItem("Tools/ProGrids/Decrease Grid Size _[", false, 202)]
	...
	[MenuItem("Tools/ProGrids/Increase Grid Size _]", false, 203)]
	...
	// [MenuItem("Tools/ProGrids/Nudge Perspective Up _&[", true, 204)]
	// [MenuItem("Tools/ProGrids/Nudge Perspective Up _&]", true, 205)]
	// public static bool VerifyMenuNudgePerspective()
	// {
	// 	return instance != null && !instance.fullGrid && !instance.ortho && instance.lockGrid;
	// }

	[MenuItem("Tools/ProGrids/Nudge Perspective Negative _-", false, 204)]
	...
	[MenuItem("Tools/ProGrids/Nudge Perspective Positive _=", false, 205)]
...
#region INITIALIZATION
	...
	{
		#if DEBUG
		pivotGo = GameObject.CreatePrimitive(PrimitiveType.Cube);
		scenePivotGo = GameObject.CreatePrimitive(PrimitiveType.Cube);
		scenePlaneIntercept = GameObject.CreatePrimitive(PrimitiveType.Cube);
		#endif
...
#region INTERFACE
	...
	GUIStyle toggleStyle, pgStyle, snapToGridStyle;
	...
	{	
		if(!guiInitialized) 
		{
			...
			pgStyle = new GUIStyle();
			toggleStyle = new GUIStyle();
			snapToGridStyle = new GUIStyle();

			toggleStyle.alignment = TextAnchor.MiddleLeft;
			toggleStyle.margin = new RectOffset(4,2,0,0);
			// toggleStyle.normal.background = t_skin.GetStyle("FloatField").normal.background;
			// pgStyle.padding = new RectOffset(2,2,1,0);
			// pgStyle.border = new RectOffset(6, 6, 4, 4);
			pgStyle.normal.background = null;//t_skin.GetStyle("Button").normal.background;
			pgStyle.margin = new RectOffset(0,0,0,0);
			pgStyle.contentOffset = new Vector2(0,0);

			snapToGridStyle.normal.background = null;//gui_SnapToGrid;
			snapToGridStyle.active.background = gui_SnapToGrid_pressed;
			snapToGridStyle.margin = new RectOffset(0,0,0,0);
			snapToGridStyle.contentOffset = new Vector2(0,0);
		}
		...
		if(GUILayout.Button(gc_LockGrid, pgStyle))
		{
			lockGrid = !lockGrid;
			SceneView.RepaintAll();
		}
...
#region ONSCENEGUI
	...
	const string AXIS_CONSTRAINT_KEY = "c";
	private bool axisConstraintKey = false;
	...
	public void OnSceneGUI(SceneView scnview)
	{
		...
		{
			axisConstraintKey = true;
		}

		if(e.type == EventType.KeyUp)
			axisConstraintKey = false;
		...
		else
		{
			...
			{
				// Bugger.SetKey("dist\ngrid", dist + "\n" + planeGridDrawDistance );
				sceneViewPlanePivot = ray.GetPoint( Mathf.Min(dist, planeGridDrawDistance/2f) );				
			}
			else
			{
				// The Min(dist, plaeGridDrawDistance) prevent near clip issues when drawing grid
		...
		{		
			if(lastTransform.position != lastPosition)
			{
				...
				if(useAxisConstraints || axisConstraintKey)
...
#region SETTINGS
	...
	public void SetSnapValue(SnapUnit su, float val)
	{
		...
		EditorPrefs.SetFloat(pg_Constant.SnapValueEditorPref, val);
